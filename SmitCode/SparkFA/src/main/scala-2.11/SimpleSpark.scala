...
object SimpleSpark {
  ...
  def main(args: Array[String]) {
    ...
    //10 100 10 LassoWithSGD 100 1001L
    ...
    //--------------------------------------------------------------------------------------------
    //Variables
    val financialRiskAnalysisParam = args(0) //10 //toInt(z.input("Financial RiskAnalysis Param in %").toString)
    val nTrials = args(1) //100 //toInt(z.input("Number Of Trials (Min 100)").toString)
    val slidingWindow = args(2) //10 //toInt(z.input("Time interval").toString)

    val algoNAme = args(3)// "LassoWithSGD" //z.input("Regression Algorithm To be used  (Values : LinearRegressionWithSGD_L0,LinearRegressionWithSGD_L1,LinearRegressionWithSGD_L2,RidgeRegressionWithSGD,LassoWithSGD)").toString
...
//      arrayElement filter {
//        case (x: String, y: Double) => x == "ShanghiaCompositeIndex"
//      }
//    }
    ...
    val numTrials = args(4).toInt //100 //nTrials.getOrElse(0)
    val parallelism = numTrials / 10
    val baseSeed = 1001L //args(5).toLong //1001L
    ...
    //val myFilePrefix = "/home/hadoop/data/StockNamesMap.csv"
